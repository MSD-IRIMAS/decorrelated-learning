...
def get_args():
    ...
    parser.add_argument('--output-directory', help='output directory parent', type=str, default='results_new/co_models_2/',)
...
def train_alone_model(model, base_model_1, epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        penalty = torch.mean(torch.norm(dot_prod_no_diog, p='fro', dim=(1, 2)) / (torch.count_nonzero(dot_prod_no_diog, dim=(1,2)) +1 )) / 40        
...
if __name__ == '__main__':
    ...
    base_seeds_1 = [2344, 1295, 6258, 6742, 3319]   
    co_seeds =     [1295, 6258, 6742, 3319, 2344]    
    
     
    for base_seed_1, co_seed in zip(base_seeds_1, co_seeds):
        ...
        for dataset in args.datasets:
            ...
            output_directory = os.getcwd() + '/' + output_directory + f'co_seed_{co_seed}_base_seed_{base_seed_1}/' + dataset + '/'
            ...
            base_model_1.load_state_dict(torch.load(f'results/base_seed_{base_seed_1}/' + dataset + '/best_model.pt'))
            base_model_1.cuda()
            base_model_1.eval()
