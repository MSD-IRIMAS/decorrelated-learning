...
def get_args():
    ...
    parser.add_argument('--output-directory', help='output directory parent', type=str, default='results_new/co_models_5/',)

    args = parser.parse_args()

    return args

def train_alone_model(model, base_model_1, base_model_2, base_model_3, base_model_4, epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        _, features_base_3 = base_model_3.extract_features(inputs.float())
        _, features_base_4 = base_model_4.extract_features(inputs.float())
        ...
        last_layer_feat_base_3 = features_base_3[-1]   
        last_layer_feat_base_4 = features_base_4[-1]   
        ...
        dot_product_3 = torch.matmul(last_layer_feat_co, last_layer_feat_base_3.transpose(2, 1).detach())        
        mask_3 = 1 - torch.eye(num_features, device=dot_product.device).unsqueeze(0)
        dot_prod_no_diog_3 = dot_product_3 * mask_3
        
        penalty_3 = torch.mean(torch.norm(dot_prod_no_diog_3, p='fro', dim=(1, 2)) / (torch.count_nonzero(dot_prod_no_diog_3, dim=(1,2)) +1 )) / 40        


        dot_product_4 = torch.matmul(last_layer_feat_co, last_layer_feat_base_4.transpose(2, 1).detach())        
        mask_4 = 1 - torch.eye(num_features, device=dot_product.device).unsqueeze(0)
        dot_prod_no_diog_4 = dot_product_4 * mask_4
        
        penalty_4 = torch.mean(torch.norm(dot_prod_no_diog_4, p='fro', dim=(1, 2)) / (torch.count_nonzero(dot_prod_no_diog_3, dim=(1,2)) +1 )) / 40        



        penalty = (penalty_1 + penalty_2 + penalty_3 + penalty_4) / 4
...
if __name__ == '__main__':
    ...
    base_seeds_3 = [8976, 8859, 9876, 37, 5639]
    base_seeds_4 = [8859, 9876, 37, 5639, 8976]
    co_seeds = [9876, 37, 5639, 8976, 8859]

     
    for base_seed_1, base_seed_2, base_seed_3, base_seed_4, co_seed in zip(base_seeds_1, base_seeds_2, base_seeds_3, base_seeds_4, co_seeds):
        ...
        for dataset in args.datasets:
            ...
            output_directory = os.getcwd() + '/' + output_directory + f'co_seed_{co_seed}_base_seed_{base_seed_4}_{base_seed_3}_{base_seed_2}_{base_seed_1}/' + dataset + '/'
            ...
            base_model_3 = LITE(output_directory=None, length_TS=length_TS, n_classes=len(np.unique(ytrain)), n_filters=32)
            base_model_3.load_state_dict(torch.load(f'results_new/co_models_3/co_seed_{base_seed_3}_base_seed_{base_seed_2}_{base_seed_1}/' + dataset + '/best_model.pt'))
            base_model_3.cuda()
            base_model_3.eval()
            
            base_model_4 = LITE(output_directory=None, length_TS=length_TS, n_classes=len(np.unique(ytrain)), n_filters=32)
            base_model_4.load_state_dict(torch.load(f'results_new/co_models_4/co_seed_{base_seed_4}_base_seed_{base_seed_3}_{base_seed_2}_{base_seed_1}/' + dataset + '/best_model.pt'))
            base_model_4.cuda()
            base_model_4.eval()
            ...
            for epoch in range(epochs):
                ...
                train_loss, ce_loss, loss_div_feat, train_acc = train_alone_model(clf, base_model_1, base_model_2, base_model_3, base_model_4, epoch)
