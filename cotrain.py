...
def get_args():
    ...
    parser.add_argument('--output-directory', help='output directory parent', type=str, default='results/co_models/',)

    args = parser.parse_args()

    return args

def train_alone_model(model, base_model_1, epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        _, features_base = base_model_1.extract_features(inputs.float())
        
        last_layer_feat_co = features_co[-1]
        last_layer_feat_base = features_base[-1]        

        dot_product = torch.matmul(last_layer_feat_co, last_layer_feat_base.transpose(2, 1))        
        ...
        penalty1 = torch.mean(torch.norm(dot_prod_no_diog, p='fro', dim=(1, 2)) / (torch.count_nonzero(dot_prod_no_diog, dim=(1,2)) +1 )) / 40

        loss_ce_ = criterion_CE(outputs, targets)
        loss = 0.5 * loss_ce_ + 0.5 * penalty1

        loss.backward()
        optimizer.step()        

        loss_div_feat += penalty1
        loss_ce += loss_ce_
        loss_total += loss
...
if __name__ == '__main__':
    ...
    base_seeds = [37, 5639, 8976, 8859, 9876]   
    co_seeds = [5639, 8976, 8859, 9876, 37]    
     
    for base_seed, co_seed in zip(base_seeds, co_seeds):
        ...
        for dataset in args.datasets:
            ...
            output_directory = os.getcwd() + '/' + output_directory + f'co_seed_{co_seed}_base_seed_{base_seed}/' + dataset + '/'
            ...
            base_model_1.load_state_dict(torch.load(f'results/base_seed_{base_seed}/' + dataset + '/best_model.pt'))
            base_model_1.cuda()
            base_model_1.eval()
            ...
            for epoch in range(epochs):
                ...
                train_loss, ce_loss, loss_div_feat, train_acc = train_alone_model(clf, base_model_1, epoch)
